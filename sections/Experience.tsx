...
const experiences = [
  {
    ...
    bgColor: "bg-primary",
    shadowColor: "rgba(143, 245, 255, 1)",
  },
  {
    ...
    shadowColor: "rgba(172, 137, 255, 1)",
  },
  {
    ...
    bgColor: "bg-tertiary",
    shadowColor: "rgba(137, 149, 255, 1)",
  },
];
            ...
            {experiences.map((exp, index) => (
              ...
              >
                <div className={`w-[45%] ${exp.align === "left" ? "text-right pr-12" : "text-left pl-12"}`}>
                  ...
                  <p className={`${exp.textColor} text-sm font-headline mb-3`}>{exp.period}</p>
                  <p className="text-on-surface-variant text-sm">{exp.description}</p>
                </div>

                <div
                  className={`absolute left-1/2 -translate-x-1/2 w-4 h-4 rounded-full ${exp.bgColor} z-10`}
                  style={{ boxShadow: `0 0 15px ${exp.shadowColor}` }}
                />
